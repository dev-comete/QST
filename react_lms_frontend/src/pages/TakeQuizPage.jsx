...
export default function TakeQuizPage() {
  ...
  // 🌟 2. On récupère le vague_id depuis l'URL (?vague_id=...)
  ...
  useEffect(() => {
    // 🌟 Sécurité : on bloque si le vague_id est manquant
    if (!vagueId) {
      setError("Erreur : La session de formation (vague) est introuvable. Veuillez repasser par le tableau de bord.");
      setLoading(false);
      return;
    }
    ...
    const fetchQuizData = async () => {
      try {
        // 🌟 3. On passe l'ID de la vague au service de démarrage
        const data = await StudentQuizService.startQuiz(id, vagueId);
  ...
  // 🌟 4. Mise à jour de la soumission automatique (Timeout)
  const forceSubmitTimeout = async () => {
    setSubmitting(true);
    try {
      const payload = {
        quiz_id: parseInt(id, 10),
        vague_id: parseInt(vagueId, 10),
  ...
  // 🌟 5. Mise à jour de la soumission manuelle
