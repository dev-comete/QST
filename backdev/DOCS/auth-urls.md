...
The custom JWT login API is defined separately in `backdev/accounts/urls.py` at `/accounts/auth/login/`.
...
| :--- | :---: | :---: | :--- |
| `/accounts/auth/login/` | `POST` | Not required | Custom API login endpoint using `CustomLoginAPIView` and JWT token issuance. |
...
- The custom API login endpoint is separate and defined in `backdev/accounts/urls.py` at `/accounts/auth/login/`.
